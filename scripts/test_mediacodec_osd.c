...
static void test_prefetch_retains_warming_until_serviced(void)
{
    struct osd_prefetch p = static_prefetch();
    osd_prefetch_store(&p, 10.0, 1, 3, 2, 0.01);
    osd_prefetch_retire(&p, 9.999, 3);
    CHECK(p.valid, "the image remains available for GPU warming before its start");
    osd_prefetch_retire(&p, 10.0, 3);
    CHECK(!p.valid, "servicing the boundary releases the staging slot");

    osd_prefetch_store(&p, 20.0, 1, 3, 2, 0.01);
    osd_prefetch_retire(&p, 20.125, 3);
    CHECK(!p.valid, "a coalesced request also releases the staging slot");

    osd_prefetch_store(&p, 30.0, 1, 3, 2, 0.01);
    osd_prefetch_retire(&p, 5.0, 4);
    CHECK(!p.valid, "a backwards seek releases a future image from the old epoch");
}
...
static void test_full_result_queue_preserves_pending_releases(void)
{
    struct osd_result_queue q = {0};
    for (int n = 0; n < OSD_RESULT_CAPACITY; n++) {
        CHECK(osd_result_push(&q, (struct osd_result){
            .slot = n, .seq = n + 1,
        }), "accepted snapshots fill the queue");
    }
    CHECK(!osd_result_push(&q, (struct osd_result){.slot = 99, .seq = 99}),
          "a full queue refuses admission instead of replacing a pending pose");
    CHECK(osd_result_pop(&q).seq == 1, "the oldest pose survives overflow");
    CHECK(osd_result_push(&q, (struct osd_result){
        .slot = OSD_RESULT_CAPACITY, .seq = OSD_RESULT_CAPACITY + 1,
    }), "consumption makes room for the next pose");

    // Publication must reach the tail even after its array index wraps.
    osd_result_publish_release(&q, (struct osd_release){
        .seq = OSD_RESULT_CAPACITY + 1, .timestamp = 123456,
    });
    for (int n = 1; n <= OSD_RESULT_CAPACITY; n++) {
        struct osd_result r = osd_result_pop(&q);
        CHECK(r.seq == (uint64_t)n + 1 && r.slot == n,
              "overflow and ring wrap preserve every queued pose in order");
        CHECK(r.release.timestamp == (n == OSD_RESULT_CAPACITY ? 123456 : 0),
              "release publication affects only the matching wrapped pose");
    }
}
...
int main(void)
{
    ...
    test_prefetch_retains_warming_until_serviced();
    test_prefetch_streak_counts_unchanged_renders();
    test_full_result_queue_preserves_pending_releases();
